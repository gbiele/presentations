...
drawmydag(collider)
```
Estimating the effect of distance to work out place on fitness will be biased if participation in the follow up (FU) measurement depends on that distance and unobserved education influences both distance and fitness. 
:::
...
drawmydag(collider)
```
In an RCT where medication status influences side effects (Si), which influence drop out (DO), and unobserved comorbid disorders influence side effects and symptoms, an unbiased causal effect cannot be estimated.
:::
::::

**Experiments with drop out require, just as observational studies, the assumption of no unobserved confounding!**
...
:::: {.columns}
::: {.column width="30%"}
Kinge et al.,<b> Parental income and mental disorders in children and adolescents: prospective register-based study.</b> IJE (2021). <br> "...associations between lower parental income and children’s mental disorders were partly, but not fully, attributed to other socio-demographic factors..." 
:::
::: {.column width="70%"}
...
- Causal inference is hard, and experiments do not guarantee unbiased causal effects
  - randomized treatment assignment is not enough, we also need truly random samples and relevant outcome
- Admittedly, causal inference with observational data is even harder, but there are designs with higher internal validity
